...
declare global {
  interface HTMLElementTagNameMap {
    'ak-font-cipher': AkFontCipher;
  }
}

@localized()
@customElement('ak-font-cipher')
export class AkFontCipher extends LitElement {
  static styles = [
    css`
      :host {
        display: block;
  ...
  protected render() {
    return html`<h1 class="text-3xl">${msg(html`Font Cipher`)}</h1>
      <p class="text-xl mb-12">
        ${msg(
          html`Generate customized fonts for displaying text content encrypted
          with Caesar cipher with specified shift number.`,
        )}
      </p>
      <div class="flex flex-col gap-4">
        <div class="flex flex-col gap-2">
          <label for="original-text">${msg(html`Original text`)}</label>
          <textarea
            id="original-text"
            class="border-none rounded-sm p-2 break-all"
            .value=${this.originalText}
            @input=${this.onOriginalTextChange}
            @blur=${this.encrypt}
          ></textarea>
          <span class="text-neutral-4 text-sm -mt-1">
            ${msg(html`Count: ${this.originalText.length}`)}
          </span>
        </div>
        <div class="flex flex-col gap-2">
          <label for="shift-number">${msg(html`Shift number`)}</label>
          <input
            id="shift-number"
            class="border-none rounded-sm p-2"
            type="number"
            min="1"
            max="94"
          ...
          />
          <span class="text-neutral-4 text-sm -mt-1">
            ${msg(html`From 1 to 94`)}
          </span>
        </div>
        <div class="flex flex-col gap-2">
          <label for="encrypted-text">${msg(html`Encrypted text`)}</label>
          <textarea
            id="encrypted-text"
            class="border-none rounded-sm p-2 break-all"
        ...
        <div class="flex flex-col gap-2">
          <label for="font-file">${msg(html`Font file (.ttf)`)}</label>
          <input
            id="font-file"
  ...
  private renderFontDownload() {
    ...
    return html`<p>${msg(html`Download the generated new fonts`)}</p>
